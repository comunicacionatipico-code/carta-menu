      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 overflow-y-auto p-4 max-w-xl">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4">
            <p className="text-sm font-medium text-gray-700 mb-1">🌐 Dominio personalizado</p>
            <p className="text-xs text-gray-400 mb-3">
              Conecta tu propio dominio o subdominio para que la carta se sirva desde ahí.
              Ejemplo: <span className="font-mono">carta.elsocarrat.es</span>
            </p>
            <input
              type="text"
              value={data.dominio_personalizado ?? ''}
              onChange={e => setData(prev => ({ ...prev, dominio_personalizado: e.target.value.trim().toLowerCase() || undefined }))}
            ...
            {data.dominio_personalizado && (
              <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700 space-y-1">
                <p className="font-semibold">Pasos para activarlo:</p>
                <p>1. En tu proveedor DNS añade un registro <span className="font-mono font-bold">CNAME</span>:</p>
                <p className="font-mono ml-4">{data.dominio_personalizado.split('.').slice(0, -2).join('.')} → cname.vercel-dns.com</p>
                <p>2. En Vercel → tu proyecto → Domains → añade <span className="font-mono font-bold">{data.dominio_personalizado}</span></p>
                <p>3. Guarda cambios aquí. En ~24h funcionará.</p>
              </div>
            )}
          </div>
